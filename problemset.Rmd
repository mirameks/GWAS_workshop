...
```{r}
data<-read.table("~/Desktop/GWAS_workshop/CB1908_L2_IC50.assoc.linear",header=T)
...
### Up until the 45th percentile, the data points align somewhat closely with the expected distribution. Towards the end, it starts to deviate farther than the expected line indicating that these SNPs' p-values are much more significant and is associated with the drug response to CB1908. There are also possibilities of confounders because the values start to deviate in the middle rather than at the very end. We should investigate other potential genetic factors that could influence these SNPs differences such as their genetic makeup. We should also look at the condition and the environment these individuals were treated in.

5.	Do you have any genome-wide significant hits?  How did you correct for multiple hypothesis testing?  

### I learned that Bonferroni correction is one of the methods used to correct for multiple hypothesis testing. This was already provided in the adjusted linear association values file. The data that stuck out to me was in Chromosome 12, the Bonferroni correction value is less than 1 by a lot (1.313e-05), so the SNPs on chromosome 12 show the strongest association to the drug response than the other chromosomes. 
...
### The top SNP (rs10876043) is located on chromosome 12 and shows the strongest association with drug response from CB1908. In the database I used, it is mentioned that this rsID is an intron variant, so this SNP is within the gene, however, it is the non-coding part of the gene. In response to CB1908, these loci may affect gene regulation by influencing the proteins being made due to differences in how exons are spliced to express that gene. This could change how the drug reacts with the protein due to changes in the drug binding sites. 
...
###using plink, pull out the genotypes of this rsID to use appropriately in R

./plink --bfile genotypes --snp rs10876043 --recode A 
  --out rs10876043_genotypes

```{r}
gts<-read.table("~/Desktop/GWAS_workshop/rs10876043_genotypes.raw"
  ,header=T)
pts<-read.table("~/Desktop/GWAS_workshop/CB1908_IC50_log2.txt"
  ,header=T)
boxplot(pts$log2_CB1908_IC50~gts$rs10876043_T
  ,ylab="CB1908 log2(IC50)",xlab="rs10876043 genotype")
```

###this creates a box plot (rs10876043 is not a separate file in the GWAS workshop, so I am not sure how this would work, but if it was a file, then this code would be used to create the boxplot)

8.	What analyses and experiments would you do next to follow-up on your findings?
### we could experiment to see how nearby genes could be affected by the drug and how that affects gene regulation. We could experiment again with a more uniform group with close to the same genetic makeup to exclude confounding variables. Also figure out how exactly the drug affects the body and the role of the chromosome 12
